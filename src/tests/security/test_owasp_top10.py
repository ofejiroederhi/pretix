...
def pytest_collection_modifyitems(session, config, items):
    """Log how many OWASP tests were collected (hypothesis H4: deselection)."""
    our = [i for i in items if "test_owasp_top10" in i.nodeid]
    if our:
        _debug_log("security items collected", {"count": len(our), "nodeids": [i.nodeid for i in our[:3]]}, "H4")
# #endregion
